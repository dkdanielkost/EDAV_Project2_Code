...
##Top 10 Country Investigation
  ...
  Now we will investigate the top 10 countries within different categoris of the flood data to see if we can visualize any unique characteristics within the data. We will look at variables such as the number of floods, flooding deaths and number of people displaced by flooding.
...
Clearly, there are conclusions to be drawn by examining some of these plots. Right away, we can observe that there are several countries that appear in all three plots: Brazil, China, India, Indonesia, Philippines, USA and Vienam. Seeing some of the more devloped countries such as the USA might be surprising but there could be a difference in reporting. Another interesting find is that Iran, Pakistan and Afghanistan appear in the top 10 for deaths but not for displacements.

* In Iran there were 9,107 deaths and 10,364 displaced.

* In Pakistan there were 9,346 deaths and 14,611 displaced.

* In Afghanistan there were 9,150 deaths and 10,931 displaced.

This can be explained by the positive correlation between the number of Flood Events, Deaths and Displaced.
...
As we look at the right side of the plots "Groupd by Month" we can further see some interesting characteristics. Looking at the upper right plot it would seem that for the world most of the flooding occurs in July. However, as we break this down to Northern and Southern Hemispheres we can see that these seasons are opposite for each region. This makes sense because the different hemispheres experience opposite seasons. Severity per month does not tell us much more additional information as the splits are very similar.
...
An interesting find from this graph is the outlier that was located at the top for Duration in Days. We can see that there is a flood event that happend in the USA and lasted for 419 days. The next closest was 168 days which occured in Zambia. Another interesting observation is the negative correlation between elevation and the duration in days. This was removed in the plotting due to resolution.
...
From these plots we can notice that there is a negative correlation between elevation and the number of displaced people for the flood events. In contract, we observe a positive correlation between elevation and the number of dead. 

For Affected Square km it might seem that there is no correlation but there is a slightly negative correlation between the Elevation and Affected Square km. the Y-axis is suppressing this but it is not a strong enough correlation to be relevant.
#
...
First we will take a holistic view of the world map during 2003, which was the year that generated the largest number of floods. While this could have been just due to reporting, we will focus on this year to narrow the scope of the data.
...
We then did a PCA analysis on the 2003 data, with 10 numerical variables of "Duration.in.Days", "Dead", "Displaced", "Severity..", "Affected.sq.km", "Magnitude..M...", "Centroid.X", "Centroid.Y", "M.6" and "M.4". Note that we decided to group the Main Causes of the floods. In 2003, there were 7 main causes, which were "Heavy rain", "Tropical Cyclone", "Brief torrential rain", "Dam/Levy, break or release", "Monsoonal rain", "Snow Melt", "Ice Jams", "Rain and Snow Melt" and "Rainy Season".      
...
The first principal comoponent explains quite a large proportion of the variance. After the first PC, however, the remaining principal components decrease quickly. The summary of the PCA is displayed below:

```{r, echo=FALSE,message=FALSE, warning=FALSE}
summary(pca)
```

The first PC explains 33.85% of the total variance and together with the second PC, 46% of the variance can be explained. Next, we examine the groups by first two principal components.
...
The plot shows that 8 factors (excluding the two centroids) have similar directions and they are explianed by first principle component well. Among the groups of the main causes of floods, we can see that "Snow Melt" and "Monsoonal rain" typically lie more towards the right, which indicates heavier damage. "Ice Jams" and "Brief torrential rain" are towards the left of the plot, which indicates less damage.
